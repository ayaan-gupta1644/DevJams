...
async def create_transaction(transaction: TransactionCreate, db: Session = next(get_db)):
...
async def read_transactions(db: Session = next(get_db)):
...
async def create_savings_goal(goal: SavingsGoalCreate, db: Session = next(get_db)):
    db_goal = SavingsGoal(
...
async def read_savings_goals(db: Session = next(get_db)):
